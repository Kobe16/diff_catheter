...
class SimulationExperiment:
    ...
    def execute(self):
        ...
        catheter = CCCatheter(self.p_0, self.l, self.r, self.loss_2d, self.tip_loss, self.n_mid_points, self.n_iter, verbose=0, theory=self.theory)
        ...
        catheter.calculate_beziers_control_points(init=True, noise_level=self.cc_to_bezier_noise) # 0.002
        catheter.convert_bezier_points_to_2d()
        
        print(f"ux: {catheter.ux}, uy: {catheter.uy}, l: {catheter.l}")
        ...
        if not self.use_2d_pos_target: 
            ...
            if self.loss_2d:
                print("2D target: ", catheter.target_bezier_params_list_2d)
        ...
        if self.loss_2d and not self.use_2d_pos_target:
        ...
        for i in range(self.n_iter):
            ...
            if self.use_reconstruction: 
                ...
                if i == 0:
                    optimized_bezier, opt_loss = optimize(image_save_path, cc_specs_path, i, self.images_save_dir, bezier_specs_init_torch, learning_rate=lr1)
                if self.reconstruction_mode == 2 and i > 0:
                    optimized_bezier, opt_loss = optimize(image_save_path, cc_specs_path, i, self.images_save_dir, bezier_specs_init_torch)
                if self.reconstruction_mode == 3 and i > 0:
                    ...
                    if i > 1:
                        ...
                        lr = lr3 # 2e-3, 1e-3
                    optimized_bezier, opt_loss = optimize_3_loss(image_save_path, cc_specs_path, i, self.images_save_dir, bezier_specs_init_torch, gt_img_path_list, delta_u_list, catheter.l, learning_rate=lr)
                ...
                self.optimize_loss_list.append(opt_loss)
            ...
            catheter.convert_cc_points_to_2d(i)
            catheter.calculate_beziers_control_points(i, noise_level=self.cc_to_bezier_noise) # 0.007
            catheter.convert_bezier_points_to_2d(i)
        ...
        if self.use_reconstruction:
            file_path = os.path.join(self.data_dir, 'optimize_loss.npy')
            np.save(file_path, np.array(self.optimize_loss_list))
