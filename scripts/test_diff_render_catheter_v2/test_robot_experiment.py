...
class RealRobotExperiment:
    ...
    def __init__(self, dof, loss_2d, tip_loss, interspace, viewpoint_mode, damping_weights, noise_percentage, n_iter, render_mode, n_control_iters, ux_controls, uy_controls, l_controls=None, phi_controls=None, u_controls=None):
        """
        Args:
            ...
            n_control_iters (int): number of control changes to be applied in the control lists (i.e., ux_controls, uy_controls, etc)
            ux_controls (float array): list of changes to actuation 'ux' that are applied (for 2 DoF and 3 DoF)
            uy_controls (float array): list of changes to actuation 'uy' that are applied (for 2 DoF and 3 DoF)
            l_controls (float array): list of changes to actuation 'l' that are applied (for 3 DoF)
            phi_controls (float array): list of changes to actuation 'phi' that are applied (for 1 DoF)
            u_controls (float array): list of changes to actuation 'u' that are applied (for 1 DoF)
        ...
        self.n_control_iters = n_control_iters
        self.ux_controls = ux_controls
        self.uy_controls = uy_controls
        self.l_controls = l_controls
        self.phi_controls = phi_controls
        self.u_controls = u_controls
    ...
    def execute(self):
        ...
        # Generate images of the catheter, after each control has been applied, to perform our experiments on. 
        for i in range(self.n_control_iters): 
            print('------------------------- Start of Control Iteration ' + str(i) + ' -------------------------')
            # Update the ux, uy actuation states manually
            if self.dof == 1:
                assert self.u
                assert self.phi
