...
class DoDiffOptimization(nn.Module):
    ...
    def __init__(self, para_init, para_gt, diff_model, total_itr_steps, img_raw_rgb, data_frame_id, if_print_log=1):
        ...
        self.save_data_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/real_dataset_render/loss_mask_4k/frame_' + str(self.data_frame_id)
        # self.save_data_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/real_dataset_render/loss_mask_end/frame_' + str(self.data_frame_id)
...
if __name__ == '__main__':
    ...
    for i in range(9, 10):
        ...
        # 95 - 98 : mask only
        # init_random = torch.tensor([10.0, 5.0, 0.0, 25.0, 0.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)

        # frame 104-105
        # init_random = torch.tensor([5.0, 10.0, 0.0, 50.0, 5.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)
        ## 105 : mask only
        # init_random = torch.tensor([15.0, 10.0, 0.0, 45.0, 0.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)  ## perfect good
        # init_random = torch.tensor([10.0, 14.0, 0.0, 40.0, 10.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)  ## not good
        ## 105 : mask + end
        ...
        # from frame 108-110 : mask only
        init_random = torch.tensor([8.0, 4.0, 0.0, 18.0, 5.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)

        # from frame 111--117 : mask only
        # init_random = torch.tensor([8.0, 14.0, 0.0, 18.0, 5.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)
