...
class DoDiffOptimization(nn.Module):
    ...
    def __init__(self, para_init, para_gt, diff_model, total_itr_steps, img_raw_rgb, data_frame_id, if_print_log=1):
        ...
        self.save_data_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/sim2_dataset_side1/loss_mask_4k/frame_' + str(self.data_frame_id)
...
if __name__ == '__main__':
    ...
    frame_id_list = (100, 94, 95, 97, 98, 104, 105, 108, 109, 110, 111, 112, \
                     113, 114, 115, 116, 117, 118, 119, 120, \
                     121, 122, 123, 124, 125, 135, 136, 137, \
                     138, 139, 140, 147)

    frame_id = frame_id_list[selected_frame_id]
    frame_naming = sim_case + 'side1/frame_' + (str(frame_id)).zfill(4)
    frame_id = 100
    img_save_path = frame_naming + '.png'
    img_raw_path = img_save_path
    ...
    radius_gt = np.load(sim_case + 'radius_data.npy') * 0.5
    radius_gt_3d = torch.from_numpy(radius_gt).to(gpu_or_cpu)

    cam_RT_H_mat = np.load(sim_case + 'T_side1.npy')
    cam_K_mat = np.load(sim_case + 'intrinsic_mat.npy')

    ### loading ground truth data
    gt_centline_3d = np.load(sim_case + 'position_data.npy')

    # gt_centline_3d_if_flip = np.flip(gt_centline_3d, 0).copy()
    gt_centline_3d_if_flip = gt_centline_3d.copy()

    pt0 = gt_centline_3d_if_flip[frame_id, :, 0].copy()
    pt2 = gt_centline_3d_if_flip[frame_id, :, -1].copy()
    # pt2[0] = pt0[0]
    # pt2[2] = pt0[2]
    pt1 = (pt0 + pt2) / 2

    radius_scale = np.array([1.0])
    ...
    #### ===================================================================
    init_random = torch.tensor([0.0, 0.0, 0.0, 0.000, 0.0, 0.0, 0.0], dtype=torch.float).to(gpu_or_cpu)
