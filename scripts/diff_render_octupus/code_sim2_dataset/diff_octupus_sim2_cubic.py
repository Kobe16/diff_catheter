...
class DoDiffOptimization(nn.Module):
    ...
    def __init__(self, para_init, para_gt, diff_model, total_itr_steps, if_print_log=1):
        ...
        self.optimizer = torch.optim.Adam([self.para_init], lr=1000.0)
...
if __name__ == '__main__':
    ...
    frame_naming = sim_case + 'side1/frame_0199'
    frame_id = 199
    ...
    para_init = torch.cat(
        (torch.from_numpy(pt0) + torch.rand(3) * 0, torch.from_numpy(pt1), torch.from_numpy(pt2),
         torch.from_numpy(pt3) + torch.rand(3) * 0, torch.from_numpy(np.asarray([radius_gt_3d[0], radius_gt_3d[-1]]))))
    ...
    ## cubic bezier model
    diff_model = DiffOptimizeModel(para_init=para_init[3:12],
                                   p_start=para_init[0:3],
                                   radius_start=para_init[12],
                                   radius_end=para_init[13],
