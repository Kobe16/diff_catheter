...
class DoDiffOptimization(nn.Module):
    ...
    def savingStepFigures(self, save_img_path):
        ...
        ax = axes.ravel()
        ax[0].imshow(self.diff_model.image_ref.cpu().detach().numpy().copy(), cmap=colormap, norm=colormap_norm)
        ax[0].set_title('Reference mask')
        ax[1].imshow(self.img_render_init, cmap=colormap, norm=colormap_norm)
        ax[1].set_title('Initial render mask')
        ax[2].imshow(self.diff_model.img_render_diffable, cmap=colormap, norm=colormap_norm)
        ax[2].set_title('Final render mask')
    ...
    def savingFinalStepFigures(self, save_img_path):
        ...
        # plot end effector
        ax[2].plot(self.diff_model.bezier_proj_img_npy[-1, 0], self.diff_model.bezier_proj_img_npy[-1, 1], linestyle='-', marker='o', color='#E94560', markersize=6, linewidth=1)
    ...
    def doOptimization(self):
        ...
        while self.id_iteration < self.total_itr_steps:
            ...
            save_img_steps_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/render_imgs/' + 'step_' + str(self.id_iteration) + '.png'

            save_img_loss_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/render_imgs/' + 'loss_' + str(self.id_iteration) + '.png'
            save_img_ref_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/render_imgs/' + 'reference' + '.png'

            if self.id_iteration >= self.total_itr_steps:
                self.savingFinalStepFigures(save_img_loss_path)
        ...
        save_loss_history_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/render_imgs/' + 'loss_history' + '.npy'
        saved_para_history_path = '/home/fei/diff_catheter/scripts/diff_render_octupus/torch3d_rendered_imgs/render_imgs/' + 'para_history' + '.npy'
...
if __name__ == '__main__':
    ...
    sim_case = '/home/fei/diff_catheter/data/octupus_frames/sim2/'
    frame_naming = sim_case + 'side1/frame_0100'
    frame_id = 100
    img_save_path = frame_naming + '.png'
    ...
    # centerline_gt_swap = np.load(sim_case + 'position_data.npy')
    # centerline_gt = centerline_gt_swap[:, [0, 2, 1], :]
    centerline_gt_swap = np.load(sim_case + 'position_data.npy')
    centerline_gt = centerline_gt_swap

    radius_gt = np.load(sim_case + 'radius_data.npy')
    cam_TF_mat = np.load(sim_case + 'T_side1.npy')
    # cam_TF_mat[0:3, 0:3] = np.array([[0.0000000, -1.0000000, 0.0000000], [0.0000000, 0.0000000, -1.0000000], [1.0000000, 0.0000000, 0.0000000]])

    cam_intrinsic_mat = np.load(sim_case + 'intrinsic_mat.npy')
    ...
    gt_centline_3d = np.transpose(dataset.centerline_gt[dataset.frame_id, :, :])
    radius_gt_3d = dataset.radius_gt[dataset.frame_id, :]
    radius_gt_3d = torch.from_numpy(radius_gt_3d).to(gpu_or_cpu)

    #### ===================================================================
    #      without radius + only end-effector
    #### ===================================================================
    #
    para_gt = torch.cat((torch.from_numpy(pt1), torch.from_numpy(pt2)))
    ...
    para_init = torch.cat((torch.from_numpy(pt1), torch.from_numpy(pt2))).to(gpu_or_cpu)
    ...
    diff_model = DiffOptimizeModel(para_init=para_init,
                                   ...
                                   gt_centline_3d=gt_centline_3d,
                                   cylinder_primitive_path=cylinder_primitive_path,
                                   cam_K=cam_intrinsic_mat,
                                   cam_RT_H=cam_TF_mat,
                                   gpu_or_cpu=gpu_or_cpu).to(gpu_or_cpu)

    do_diff = DoDiffOptimization(para_init=diff_model.para_init, para_gt=para_gt, diff_model=diff_model, total_itr_steps=total_itr_steps, if_print_log=1)
