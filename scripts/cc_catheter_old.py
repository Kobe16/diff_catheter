...
class CCCatheter:
    ...
    def calculate_cc_points(self, current_iter=0, init=False, target=False, noise_percentage=0):
        ...
        if target:
            ...
            # ------ Calculate the constant curvature points in theory (for initial guess in reconstruction) ------
            ...
            for i, s in enumerate(self.s_list):
                if self.mode == 1:
                    ...
                    print('[ERROR] [CCCatheter] Mode invalid')
                    exit()
                self.cc_pt_list_theory.append(p)
            
            # ------ Calculate the constant curvature points in reality (simulation) ------   
              
            # Add noise to the initial control parameters
            if init and noise_percentage > 0:
    ...
    def convert_bezier_points_to_2d(self, current_iter=0, init=False):
        """
        Convert points on the constant curvature curve to 2D points on the image taken by camera given the camera info
        ...
        if hasattr(self, 'bezier_params_optimized'):
            ...
            for i, p in enumerate(self.bezier_params_optimized):
                ...
                if self.verbose > 1:
                    print('2D Optimized Bezier Point ' + str(i + 1) + ': ')
                    print('    p = ', p_2d)
    ...
    def calculate_bezier_specs(self, init=False):
        ...
        if init:
            ...
            bezier_specs[0, :] = c + np.random.normal(0, 0.003, 3)
            bezier_specs[1, :] = p_end + np.random.normal(0, 0.003, 3)
        ...
        # if use bezier control points for feedback             
            ...
            if self.loss_2d:
                if not self.target_cc_pt_list_2d:
                    ...
                    exit()
                if not self.bezier_params_list_2d or not self.bezier_params_optimized_2d:
                    print('[ERROR] [CCCatheter] 2D bezier parameters invalid.')
                    exit()
                    
                if self.bezier_params_optimized_2d is not None: # if use reconstruction
                ...
                if self.tip_loss:
                    self.p_diffs = np.zeros((2, 1))
                    self.p_diffs[:, 0] = self.target_cc_pt_list_2d[-1] - bezier_params_list_2d[-1]

                else:
                    self.p_diffs = np.zeros((2 * len(bezier_params_list_2d), 1))

                    for i, (p, p_target) in enumerate(zip(bezier_params_list_2d, self.target_cc_pt_list_2d)):
                        self.p_diffs[i * 2:(i + 1) * 2, 0] = (p_target - p)
                ...
                if self.bezier_params_optimized is not None: # if use reconstruction
                ...
                if self.tip_loss: # Bezier end control point error
                    ...
                    print("Calculating 3D bezier tip loss (using feedback from reconstruction)")
    ...
    def calculate_beziers_control_points(self, noise_level=0, target=False):
        ...
        for i in range(n_beziers):
            ...
            # Get the middle and end points of the constant curvature curve
            # (Only consider the case where n_beziers = 1)
            self.p_mid = p_mid
            self.p_end = p_end
            ...
            if noise_level > 0:
                ...
                p_mid_noisy = p_mid + self.generate_noise(p_mid, noise_level)
                p_end_noisy = p_end + self.generate_noise(p_end, noise_level)
                
                self.p_mid = p_mid_noisy
                self.p_end = p_end_noisy

                c = (p_mid_noisy - (p_start / 4) - (p_end_noisy / 4)) * 2
                c1_noisy = 4 / 3 * p_mid_noisy - 1 / 3 * p_end_noisy
                c2_noisy = 4 / 3 * p_mid_noisy - 1 / 3 * p_start
                 # 4 bezier control points, for rendering catheter with Blender
                self.bezier_set.enter_spec(p_start, p_end_noisy, c1_noisy, c2_noisy)
                
                # 3 bezier control points, for control loop
                b1_noisy = (p_mid_noisy - (self.p_0 / 4) - (p_end_noisy / 4)) * 2
                self.bezier_params_list.append(b1_noisy)
                self.bezier_params_list.append(p_end_noisy)
