...
Use centerline loss instead of contour loss
"""
...
class ReconstructionOptimizer(nn.Module): 
    ...
    def forward(self, save_img_path): 
        ...
        loss_centerline = self.centerline_loss(self.build_bezier.bezier_proj_centerline_img.to(gpu_or_cpu), self.ref_catheter_centerline.to(gpu_or_cpu))
        ...
        for i in range(len(predicted_paras)):
            ...
            loss_centerline_m = self.centerline_loss(construction_bezier.bezier_proj_centerline_img.to(self.gpu_or_cpu), self.centerline_list[i].to(self.gpu_or_cpu))
            motion_model_loss += loss_centerline_m.to(self.gpu_or_cpu)
        ...
        loss = loss_centerline.to(self.gpu_or_cpu) * weight[0] + loss_tip_distance.to(self.gpu_or_cpu) * weight[1] + motion_model_loss * weight[2] / len(predicted_paras)
        ...
        # TODO: Plot the loss
        self.loss = loss
        self.loss_contour = loss_centerline
