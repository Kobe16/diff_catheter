...
def plot_3D_bezier_curve(control_points, control_points_gt=None, control_points_init=None, save_path=None, equal=False, show=True):
    """
...
def plot_2d_end_effector_loss(proj_end_effector_loss_history, full_path=None, show=True):
...
def plot_3d_mid_control_point_loss(d3d_mid_control_point_loss_history, full_path=None, show=True):
...
def plot_total_loss(loss_history, full_path=None, log_scale=False, show=True):
...
def plot_contour_loss(loss_contour_history, full_path=None, log_scale=False, show=True):
...
def plot_motion_model_loss(loss_motion_model_history, full_path=None, log_scale=False, show=True):
    ...
    if log_scale:
        ax2.set_ylim(bottom=1)
        ax2.set_yscale('log')
        ax2.set_ylabel('Motion Model Loss (log scale)')
    ax2.grid(True)
...
def plot_3D_bezier_curve_series(bezier_list, save_path=None, equal=False, show=True):
    """
    bezier_list: list of numpy arrays, each containing control points in the standard format.
    The first, penultimate, and last curves will be highlighted with different colors.
    """
    ...
    # Plot initial curve
    control_points_init = bezier_list[0]
    curve_init = bezier_curve_3d(control_points_init)
    ax.plot(control_points_init[:, 0], control_points_init[:, 1], control_points_init[:, 2], 'go--')  # Control points
    ax.plot(curve_init[:, 0], curve_init[:, 1], curve_init[:, 2], 'g-', label='Initial Curve')

    # Plot intermediate curves
    for control_points in bezier_list[1:-2]:
        curve = bezier_curve_3d(control_points)
        ax.plot(control_points[:, 0], control_points[:, 1], control_points[:, 2], 'ko--')  # Control points
        ax.plot(curve[:, 0], curve[:, 1], curve[:, 2], 'k-')  # Bezier curve

    # Plot penultimate (final curve) curve
    control_points_penultimate = bezier_list[-2]
    curve_penultimate = bezier_curve_3d(control_points_penultimate)
    ax.plot(control_points_penultimate[:, 0], control_points_penultimate[:, 1], control_points_penultimate[:, 2], 'bo--')  # Control points
    ax.plot(curve_penultimate[:, 0], curve_penultimate[:, 1], curve_penultimate[:, 2], 'b-', label='Final Curve')

    # Plot final (target) curve
    control_points_final = bezier_list[-1]
    curve_final = bezier_curve_3d(control_points_final)
    ax.plot(control_points_final[:, 0], control_points_final[:, 1], control_points_final[:, 2], 'ro--')  # Control points
    ax.plot(curve_final[:, 0], curve_final[:, 1], curve_final[:, 2], 'r-', label='Target Curve')

    # Set labels
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Catheter Trajectory')
